  ...
  .controller('AppsController', function($scope, $http, $interval, $timeout, $window) {
    ...
    apps.setDonutUX = function(data) {
      $scope.data = [
        ...
        { value: data.GOBPCount, color: '#54351c', highlight: '#54351c', label: 'Go'},
        ...
        { value: data.BinaryBPCount, color: '#298390', highlight: '#5eb5d10', label: 'Binary'},//TODO color
        { value: data.StaticFileBPCount, color: '#F7464A', highlight: '#5eb5d10', label: 'StaticFile'},//TODO color
        { value: data.ExternalBPCount, color: '#7874af', highlight: '#545483', label: 'External'},
    ...
    apps.getAppsInfo = function() {
      ...
      responsePromise.success(function(data, status, headers, config) {
        ...
        $scope.runningMemory = data.RunningMemory || messaging.empty;
        $scope.runningDisk = data.RunningDisk || messaging.empty;
        ...
        $scope.binaryBPCount = data.BinaryBPCount || messaging.zero;
        $scope.staticFileBPCount = data.StaticFileBPCount || messaging.zero;
        $scope.otherBPCount = data.OtherBPCount || messaging.zero;
        $scope.externalBPCount = data.ExternalBPCount || messaging.zero;
        $scope.stoppedStateCount = data.StoppedStateCount || messaging.zero;
        $scope.failedInStoppedStateCount = data.FailedInStoppedStateCount || messaging.zero;
        $scope.startedStateCount = data.StartedStateCount || messaging.zero;
        $scope.diegoAppsCount = data.DiegoAppsCount || messaging.zero;
        $scope.orgCount = data.orgCount || messaging.zero;
      ...
      responsePromise.error(function(data, status, headers, config) {
        ...
        $scope.runningMemory = messaging.empty;
        $scope.runningDisk = messaging.empty;
        ...
        $scope.binaryBPCount = messaging.empty;
        $scope.staticFileBPCount = messaging.empty;
        $scope.otherBPCount = messaging.empty;
        $scope.externalBPCount = messaging.empty;
        $scope.stoppedStateCount = messaging.empty;
        $scope.failedStoppedStateCount = messaging.empty;
        $scope.startedStateCount = messaging.empty;
        $scope.diegoAppsCount = messaging.empty;
        $scope.orgCount = messaging.empty;
